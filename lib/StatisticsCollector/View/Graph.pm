...
sub process {
    ...
    $graph->set(
        ...
        y_max_value   => $y_max,
        y_min_value   => $y_min,
        y_tick_number => $ticks-1,
        y_long_ticks  => 1,
        
        dclrs         => [ undef, 'dgreen' ],
        borderclrs    => [ undef, 'black' ],
        transparent   => 0,
        bar_spacing   => 2,
    );
    
    my @data;
    push @data, $c->stash->{labels};
    push @data, $_->[0] for @{ $c->stash->{data} };
    $graph->plot(\@data);
    
    my $format = $graph->export_format;
    
    $c->log->warn("Format: $format");
    
    $c->response->body($graph->gd->$format);
    $c->response->content_type("image/$format");
}
